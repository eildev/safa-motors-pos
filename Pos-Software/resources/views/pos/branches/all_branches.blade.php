                                ...
                                @if ($branches->count() > 0)
                                    @foreach ($branches as $key => $branch)
                                        <tr>
                                            ...
                                            <td>
                                                {{-- <img src="{{asset('uploads/branch/'. $branch->logo)}}" alt="logo" height="60px" width="60px"> --}}
                                                <img src="{{ file_exists(public_path('uploads/branch/' . $branch->logo)) && $branch->logo ? asset('uploads/branch/' . $branch->logo) : asset('dummy/image.jpg') }}"
                                                    alt="logo" height="60px" width="60px">
                                            </td>
                                            <td>
                                                @if (Auth::user()->can('branch.add'))
                                                    <a href="{{ route('branch.edit', $branch->slug) }}"
