    ...
    <div class="row">
        @if (Auth::user()->can('employee-salary.add'))
            <div class="col-md-12 grid-margin stretch-card d-flex justify-content-end">
                <div class="">
                    <h4 class="text-right"><a href="{{ route('employee.salary.add') }}" class="btn btn-info">Add New Salary
                        </a></h4>
                </div>
            </div>
        @endif
                                ...
                                @if ($employeSalary->count() > 0)
                                    @foreach ($employeSalary as $key => $employeSalarys)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>
                                                <a href="{{ route('employee.details', $employeSalarys->employee_id) }}">
                                                    {{ $employeSalarys['emplyee']['full_name'] ?? '' }}
                                                </a>
                                            </td>
                                            <td>{{ $employeSalarys['branch']['name'] ?? '' }}</td>
                                            <td>{{ \Carbon\Carbon::parse($employeSalarys->date)->format('d F Y') ?? '' }}
                                            </td>
                                            <td>{{ $employeSalarys->debit ?? '' }}</td>
                                            <td>{{ $employeSalarys->creadit - $employeSalarys->debit ?? '' }}</td>
                                            <td>{{ $employeSalarys->created_at->format('d F Y') ?? '' }}</td>
                                            <td>{{ $employeSalarys->updated_at ? $employeSalarys->updated_at->format('d F Y') : '-' }}
                                            </td>
                                            <td>
                                                @php
                                            ...
                                            <td>
                                                @if (Auth::user()->can('employee-salary.edit'))
                                                    <a href="{{ route('employee.salary.edit', $employeSalarys->id) }}"
                                                        class="btn btn-sm btn-primary btn-icon" title="Edit Data">
                                                        <i data-feather="edit"></i>
                                                    </a>
                                                @endif
                                                @if (Auth::user()->can('employee-salary.delete'))
                                                    <a href="{{ route('employee.salary.delete', $employeSalarys->id) }}"
                                    ...
                                    <tr>
                                        <td colspan="12">
                                            ...
                                            <div class="text-center">
                                                <a href="{{ route('employee.salary.add') }}" class="btn btn-primary">Add
                                                    New Sallary<i data-feather="plus"></i></a>
                                            </div>
                                        </td>
                                    </tr>
                                @endif
